...
📁 Project Structure
uPIMulator/
└── golang_vm/uPIMulator/src/device/simulator/
    ├── interconnect/          # Modules 1-3
    └── collective/            # Module 4

🧪 Test All Components
1️⃣ Interconnect Module Tests
bash# Navigate to interconnect folder
cd ~/uPIMulator/golang_vm/uPIMulator/src/device/simulator/interconnect

# Run interconnect tests
go test -v -run TestInterconnect
go test -v -run TestBasicTransfer
go test -v -run TestMultiDPU
go test -v -run TestStatistics

# Run ALL interconnect tests
go test -v -run "^(TestInterconnect|TestBasic|TestMulti|TestStatistics|TestCycle).*"

2️⃣ Router Tests
bash# Same folder: interconnect/
go test -v -run TestRouter
go test -v -run TestXYRouting
go test -v -run TestBufferless
go test -v -run TestBackpressure
go test -v -run TestMultiHop

# Run ALL router tests
go test -v -run "^TestRouter.*"

3️⃣ Mesh Network Tests
bash# Same folder: interconnect/
go test -v -run TestMesh
go test -v -run TestSimplePacket
go test -v -run TestMultiHopDelivery
go test -v -run TestAllToAll

# Run ALL mesh tests
go test -v -run "^TestMesh.*"

4️⃣ Ring AllReduce Tests
bash# Navigate to collective folder
cd ~/uPIMulator/golang_vm/uPIMulator/src/device/simulator/collective

# Run ring tests
go test -v -run TestRing
go test -v -run TestRingAllReduce
go test -v -run TestRingAllReduceMAX
go test -v -run TestRingAllReduceMIN

# Run ALL collective tests
go test -v
